...
-- ==========================================
-- MODERN UI CREATION: BLB HUB PRO
-- ==========================================
local playerGui = player:WaitForChild("PlayerGui")
local existingGui = playerGui:FindFirstChild("BLB_AutoFarm_Pro_V28")
...
local function executeAutoRepair()
    ...
    -- Teleport ke Meja Tempa
    rootPart.CFrame = craftPart.CFrame * CFrame.new(0, 0, 3)
    task.wait(0.5)
    enableFly(rootPart)

    -- Buka menu meja tempa
    triggerHoldPrompt(tempaPrompt)
    task.wait(2.0) -- Tunggu menu terbuka

    pcall(function()
        local playerGui = player:FindFirstChild("PlayerGui")
        
        -- Fungsi untuk memicu klik secara langsung pada objek GUI tanpa peduli posisi layar
        local function directFireButton(guiObject)
            if not guiObject then return end
            -- Eksekusi langsung event klik bawaan Roblox jika tersedia
            ...
            -- Fallback simulasi klik internal engine
            if firesignal then
        ...
        -- 1. CARI DAN PAKSA TEKAN TAB "PERBAIKI" BERDASARKAN TEKSNYA (Aman untuk semua resolusi layar)
        for _, gui in ipairs(playerGui:GetDescendants()) do
            if gui:IsA("TextLabel") or gui:IsA("TextButton") then
                if gui.Text == "Perbaiki" then
                    ...
                    -- Pastikan bukan tombol "Perbaiki • 1500 koin" yang ada di bawah
                    if targetBtn.AbsoluteSize.Y < 80 then -- Ukuran tombol tab di atas biasanya lebih spesifik
        ...
        task.wait(1.0) -- Jeda agar halaman tab Perbaiki terbuka

        -- 2. PILIH ALAT YANG RUSAK BERDASARKAN INDIKATOR "0/"
        ...
        -- 3. TEKAN TOMBOL "Perbaiki • 1500 koin" SECARA LANGSUNG
        for _, gui in ipairs(playerGui:GetDescendants()) do
...
-- ==========================================
-- 1. FUNGSI FARMING KHUSUS MINING (STRICT LOCK ORIGINAL)
-- ==========================================
local function farmMining(modelTarget, part, prompt, toolName, customDelay, checkToggleFunc)
    ...
    -- Logika terbukti sukses di V19: Menunggu dengan pasti menggunakan parameter UI Delay
...
-- ==========================================
-- 2. FUNGSI FARMING KHUSUS WOOD (POSISI FIX & STRICT LOCK)
-- ==========================================
local function farmWood(modelTarget, part, prompt, toolName, customDelay, checkToggleFunc)
    ...
    -- Menempatkan player 3 stud di depan dan 1.5 stud ke bawah agar menapak tanah sejajar dengan akar
    local standPos = targetPos + Vector3.new(0, -1.5, 3)
    -- Memaksa player berdiri tegak menghadap lurus ke batang pohon
    ...
    cameraConnection = RunService.RenderStepped:Connect(function()
        if targetPromptToLock and targetPromptToLock.Parent and rootPart and checkToggleFunc() then
            ...
            -- Posisi kamera diletakkan 2 stud di atas player, memandang serong ke bawah ke arah proximity
...
task.spawn(function()
    while task.wait(0.2) do
        if isRepairing then
            -- Status dihandle di dalam fungsi repair
...
StarterGui:SetCore("SendNotification", {
    ...
    Text = "Auto Repair Fully Fixed & Secure Teleport!",
    Duration = 5
})